...
def main():
    ...
    parser.add_argument("--states-to-index", type=argparse.FileType("r"), help="Path to CSV mapping states to index.")
    parser.add_argument(
        "-o",
        "--output",
    ...
    states = pd.read_csv(args.states_to_index).States
    ...
    # Save to disk.
    df[cols.values()].to_csv(args.output, index=False)
