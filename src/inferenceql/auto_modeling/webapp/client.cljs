...
(defnc pipeline
  ...
  (let [start #(fetch/post "/api/dvc" {})
    ...
    (<>
     ...
     ($ components/Pipeline {:steps steps}))))
          ...
          ($ mantine/Container
             ...
             (case active
               ...
               2 (<> ($ mantine/Title {:order 2}
                        "Statistical types")
                     ($ mantine/SimpleGrid
                        {:sx #js {:grid-template-columns "1fr repeat(2, max-content)"}}
                        (let [columns (-> text
                                          (csv/parse)
                                          (first))]
                          (for [column columns]
  ...
  (let [element (js/window.document.querySelector "#app")]
    (-> (fetch/get "/api/dvc"
                   {:accept :json
                    ...
                    :cache :no-cache})
        (.then (fn [{:keys [body]}]
                 (react-dom/render ($ app {& body}) element))))))

;; TODO: https://mantine.dev/hooks/use-interval/
(js/setInterval fetch 1000)
