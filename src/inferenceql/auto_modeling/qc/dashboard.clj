...
(defn scatter-plot-section [cols vega-type samples]
  (when (seq cols)
    (let [specs (for [col-pair cols] (scatter-plot col-pair vega-type samples))]
      {:concat specs
       :columns 3
       :spacing {:column 50 :row 50}
...
(defn spec [{sample-path :samples schema-path :schema}]
