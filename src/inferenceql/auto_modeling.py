...
def create_cgpms(df, schema,  n_models=1, parallel=True):
    ...
    The CGPM API requires a numpy array with floats and integers only. Every
    column has to be modeled -- meaning ignored columns (as specified in the
    schema) need to be removed from this array while we keep track which index
    in the numpy array correspond to which column in the original dataframe.
    ...
    """
    # XXX: those should be outsourced util functions. This has the advantage
    # for us to document them.
...
def cgpm_to_spn(states, col_name_id_mapping, schema):
    ...
    # `variables` is a dictionary mapping variable names to queryable
    # variables. `latests` is a dictionary mapping variable names to queryable
...
def cgpm_inference(
    ...
    cols = None if columns is None else [col_name_id_mapping[c]
                                         for c
                                         in columns]
